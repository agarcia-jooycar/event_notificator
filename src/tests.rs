...
mod tests {
    ...
    const AGNI: &str = "https://agni-telematic.jooycar.network";
    const INBOX: &str = "event_notificator_test";

    fn init_notificator() -> EventNotificator{
        let config = NotificatorStrategiesConfig::AgniNotificator {
            topic: INBOX.to_string(),
            environments: HashMap::new(),
            agni_config: AgniClientConfig::new(AGNI.to_string(), "event_notificator".to_string())
        };
    ...
    fn notify() {
        // Pre-conditions
        let event_notificator = init_notificator();
